...
export async function POST(request) {
  ...
  const { access_token, reference } = body;

  if (!access_token || !reference) {
    return NextResponse.json(
      {
        message: `Missing values: reference: ${reference} access_token: ${access_token}`,
      },
      { status: 400 }
    );
  }
  ...
  const config = {
    ...
    url: `${process.env.VIPPS_BASE_URL}/epayment/v1/payments/${reference}`,
    redirect: 'follow',
    headers: {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: 'Something happened while fetching payment',
        error: err.message,
      },
      { status: 500 }
    );
